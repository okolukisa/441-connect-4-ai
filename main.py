...
def draw_board(board):
    for c in range(COLS):
        for r in range(ROWS):
            pygame.draw.rect(screen, DARK_ORANGE, (c * SQUARESIZE, r * SQUARESIZE + SQUARESIZE, SQUARESIZE, SQUARESIZE))
            if board[r][c] == 0:
...
def compare_columns(item1):
    count = 0
    for i in range(ROWS):
        if (board[i][item1] != 0):
            count += 1
    return abs(item1 - 3) + count / 2
...
def minimax(board, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        valid_locations.sort(key=compare_columns)
...
while not game_over:
    ...
    if turn == AI_TURN and not game_over and not_over:
        ...
        if is_valid_location(board, col):
            ...
            if winning_move(board, AI_PIECE):
                print("You lose!")
                label = my_font.render("You lose!", 1, YELLOW)
                screen.blit(label, (40, 10))
                not_over = False
                t = Timer(3.0, end_game)
                t.start()
